					...
					$.each( ArticleList, function( key, value ) {												
						FinalContract.getValueByHash(value ,function(error, result){ /* itterate trough previus array // value = input hash */
							if(!error) { 					
								...
								$("#articlelist").append('<div class="card article"><div class="articlehash white teal-text text-lighten-2">Zgoščen člen: <span>'+value+'</span></div><div class="card-content"><p class="article-text">'+result+'</p></div><div class="card-action"><div class="row"><div class="col s12 center-align"><input type="checkbox" id="'+value+'" '+ifItDisabled+'/><label for="'+value+'" class="flow-text">'+ifItDisabled2+'</label></div></div></div></div>');
